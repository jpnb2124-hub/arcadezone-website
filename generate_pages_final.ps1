# Final generator: robust here-strings and safe replacements
...
foreach ($row in $csv) {
  ...
  # Create simple SVG placeholder using a here-string
  $escapedTitle = [System.Security.SecurityElement]::Escape($title)
  $svgContent = @'
<svg xmlns="http://www.w3.org/2000/svg" width="640" height="360">
  <rect width="100%" height="100%" fill="#f3f4f6"/>
  <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="28" fill="#111">'@
  $svgContent += $escapedTitle
  $svgContent += "`n</text>`n</svg>`n"
...
# Build sitemap using here-string header/footer
$smapHeader = @'
